...
class PlaySong extends Component {
  ...
  getThings = async () => {
    ...
    await axios
      .get(
        `http://api.napster.com/v2.2/playlists/pp.${ppid}/tracks?apikey=YTkxZTRhNzAtODdlNy00ZjMzLTg0MWItOTc0NmZmNjU4Yzk4&limit=25`
      )
      .then(thing => {
        this.setState({
      ...
      <div className={`${mood}con`}>
        ...
        <div className="border">
          <div className="songCont">
            <li className="artist">Artist: {singleSong.artistName}</li>
            <br />
            <li>Song: {singleSong.name}</li>
            <br />
            <li>Album : {singleSong.albumName}</li>
            <br />
          </div>
          ...
          <div>
            <audio
              ref="audio_tag"
              src={`${singleSong.previewURL}`}
              controls
              autoPlay
            />
            <br />
            <button onClick={this.dislike}>SKIP</button>
            <button onClick={this.addToFavorites}>LOVE</button>
            <br />
          </div>
        </div>
      </div>
